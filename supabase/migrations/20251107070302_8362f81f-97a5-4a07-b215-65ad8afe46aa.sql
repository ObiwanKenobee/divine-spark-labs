-- Create research conversations table
CREATE TABLE public.research_conversations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  title TEXT,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create research messages table
CREATE TABLE public.research_messages (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  conversation_id UUID NOT NULL REFERENCES public.research_conversations(id) ON DELETE CASCADE,
  role TEXT NOT NULL CHECK (role IN ('user', 'assistant')),
  content TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Enable RLS
ALTER TABLE public.research_conversations ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.research_messages ENABLE ROW LEVEL SECURITY;

-- RLS Policies for research_conversations
CREATE POLICY "Users can view their own conversations"
  ON public.research_conversations
  FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own conversations"
  ON public.research_conversations
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own conversations"
  ON public.research_conversations
  FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own conversations"
  ON public.research_conversations
  FOR DELETE
  USING (auth.uid() = user_id);

-- RLS Policies for research_messages
CREATE POLICY "Users can view messages from their conversations"
  ON public.research_messages
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.research_conversations
      WHERE id = conversation_id AND user_id = auth.uid()
    )
  );

CREATE POLICY "Users can create messages in their conversations"
  ON public.research_messages
  FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.research_conversations
      WHERE id = conversation_id AND user_id = auth.uid()
    )
  );

CREATE POLICY "Users can delete messages from their conversations"
  ON public.research_messages
  FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM public.research_conversations
      WHERE id = conversation_id AND user_id = auth.uid()
    )
  );

-- Create trigger for updating updated_at
CREATE TRIGGER update_research_conversations_updated_at
  BEFORE UPDATE ON public.research_conversations
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

-- Create index for better performance
CREATE INDEX idx_research_messages_conversation_id ON public.research_messages(conversation_id);
CREATE INDEX idx_research_conversations_user_id ON public.research_conversations(user_id);